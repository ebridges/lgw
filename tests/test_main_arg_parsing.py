...
@pytest.mark.parametrize(
    ...
    [
        ...
        ({"gw-deploy": True}, "lgw.main.handle_deploy_api_gateway", (MagicMock(),)),
        # gw-undeploy should call handle_undeploy_api_gateway with config
        ({"gw-undeploy": True}, "lgw.main.handle_undeploy_api_gateway", (MagicMock(),)),
        ...
        ({"domain-remove": True}, "lgw.main.handle_remove_domain", (MagicMock(),)),
        ...
        (
            ...
            ("/path/to/lambda.zip", MagicMock()),
        ),
        # lambda-deploy without --lambda-file should call handle_deploy_lambda
        # with config only
        ({"lambda-deploy": True}, "lgw.main.handle_deploy_lambda", (MagicMock(),)),
        # lambda-invoke should call handle_invoke_lambda with name and payload
        (
            {"lambda-invoke": True, "--lambda-name": "myLambda", "--payload": "data.json"},
        ...
        ({"lambda-archive": True}, "lgw.main.handle_lambda_archive", (MagicMock(),)),
    ],
)
def test_app_routing(args, expected_mock, expected_args):
    with patch("lgw.main.handle_deploy_api_gateway") as mock_deploy_api_gateway, patch(
        "lgw.main.handle_undeploy_api_gateway"
    ) as mock_undeploy_api_gateway, patch("lgw.main.handle_add_domain") as mock_add_domain, patch(
        "lgw.main.handle_remove_domain"
    ) as mock_remove_domain, patch(
        "lgw.main.handle_deploy_lambda"
    ) as mock_deploy_lambda, patch(
        "lgw.main.handle_invoke_lambda"
    ) as mock_invoke_lambda, patch(
        "lgw.main.handle_delete_lambda"
    ) as mock_delete_lambda, patch(
        "lgw.main.handle_lambda_archive"
    ) as mock_lambda_archive:
        ...
        config = expected_args[-1] if isinstance(expected_args[-1], MagicMock) else MagicMock()
        expected_args = tuple(config if arg == MagicMock() else arg for arg in expected_args)
