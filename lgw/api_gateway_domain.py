...
def add_domain_mapping(api_name, domain_name, base_path, https_certificate_arn, deploy_stage, wait_for_domain):
    ...
    if wait_for_domain:
      while True:
        ...
        if created:
          break;
        wait_for_certificate_validation(https_certificate_arn, wait_for_domain)
...
def create_domain_name(api_client, domain_name, certificate_arn):
    ...
    if response:
      ...
      if response.get('domainNameStatus') in ('UPDATING', 'PENDING'):
        info('Domain name %s is in process: [%s] %s' % (domain_name, response.get('domainNameStatus'), response.get('domainNameStatusMessage')))
        return

    response = api_client.create_domain_name(domainName=domain_name, certificateName=domain_name, certificateArn=certificate_arn)
    if response:
