...
def parse_args():
    ...
    parent_parser.add_argument(
        "--config-file",
        help="Override defaults with these settings."
    )
    ...
    lambda_deploy_parser = subparsers.add_parser(
        "lambda-deploy",
        parents=[parent_parser],
        help="Deploy a Lambda function"
    )
    lambda_deploy_parser.add_argument(
        "--lambda-file",
        help="Path to zip file with executable lambda code."
    )
    ...
    lambda_invoke_parser.add_argument(
        "--lambda-name",
        required=True,
        help="Name of the lambda to invoke."
    )
    lambda_invoke_parser.add_argument(
        "--payload",
        help="Path to a JSON file with data to send with the lambda invocation."
    )
    ...
    lambda_delete_parser.add_argument(
        "--lambda-name",
        required=True,
        help="Name of the lambda to delete."
    )
